...
class LangModelBase : public ConsGraphBuilder<ctx, MemModel, PtsTy, SubClass> {
 public:
  ...
  inline IndirectResolveOption onNewIndirectTargetResolvation(
 ...
 protected:
  ...
  [[nodiscard]] inline MapObject<KeyT, Self> *getOrAllocMapObj(
      const ctx *context, const void *tag) {
...
struct LangModelTrait<LangModelBase<ctx, MemModel, PtsTy, SubClass>> {
  ...
  [[nodiscard]] static inline llvm::StringRef getEntryName(
      const LangModelTy *model) {
    return model->getEntryName();
  }

  [[nodiscard]] static inline const llvm::Module *getLLVMModule(
      const LangModelTy *model) {
    return model->getLLVMModule();
  }

  [[nodiscard]] static inline const CallGraphNode<ctx> *getDirectNode(
      LangModelTy *model, const ctx *C, const llvm::Function *F) {
    return model->getDirectNode(C, F);
  }

  [[nodiscard]] static inline const CallGraphNode<ctx> *getDirectNodeOrNull(
      LangModelTy *model, const ctx *C, const llvm::Function *F) {
  ...
  [[nodiscard]] static inline const InDirectCallSite<ctx> *getInDirectCallSite(
      LangModelTy *model, const ctx *C, const llvm::Instruction *I) {
